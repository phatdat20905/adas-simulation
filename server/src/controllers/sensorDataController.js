...
const createSensorData = async (req, res) => {
  try {
    const { vehicleId, simulationId, timestamp, lane_status } = req.body;
    if (!vehicleId || !simulationId || !timestamp || !lane_status) {
...
const getSensorDataBySimulation = async (req, res) => {
  try {
    ...
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const sensorData = await sensorDataService.getSensorDataBySimulation(
      simulationId,
      req.user.id,
      page,
      limit,
...
const updateSensorData = async (req, res) => {
  try {
    ...
    const sensorData = await sensorDataService.updateSensorData(
      req.params.id,
      req.user.id,
      req.body,
      req.user.role
