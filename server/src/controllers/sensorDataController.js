...
const getSensorData = async (req, res) => {
  try {
    ...
    const sensorData = await sensorDataService.getSensorData(req.user.id, page, limit);
...
const getSensorDataBySimulation = async (req, res) => {
  try {
    ...
    const sensorData = await sensorDataService.getSensorDataBySimulation(simulationId, req.user.id);
    res.status(200).json({ success: true, data: sensorData });
  } catch (error) {
...
const updateSensorData = async (req, res) => {
  try {
    ...
    const sensorData = await sensorDataService.updateSensorData(req.params.id, req.user.id, req.body);
    res.status(200).json({ success: true, data: sensorData });
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  }
...
const deleteSensorData = async (req, res) => {
  try {
    const result = await sensorDataService.deleteSensorData(req.params.id, req.user.id);
